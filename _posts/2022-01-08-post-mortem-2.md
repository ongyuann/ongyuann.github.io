...
1413 - enumerated domain using powerview using techniques learnt from the syllabas - noticed a user with GenW privs, noted that down.  
...
# 旗 4
1507 - made other user 洛克了 admin on the box  
...
# 旗 5
1524 - checked 洛克了 user's bash history - nothing. copied my ssh pub key into authorized_keys file  
1610 - took me long enough to realize the privesc vector was staring at me from the 洛克了 user's home directory all along - an ansible vault file not named like oen  
...
1636 - setup my ssh pub key into authorized_keys file on the box, then grabbed 布鲁夫
...
1653 - 1659 - had remembered the krb5cc ticket's user was the one i saw earlier that had GenW privs that can lead to RBCD attack - and the standard exploitation method from the syllabus for this was via windows (mad + view + rub)  
...
1727 - add current user as 洛克了 admin and RDP user on the box  
...
2008 - apparently lat didn't work, so was checking out cifs privs by listing kerberos tickets - key lessin: see who the "client" is when klisting
...
2228 - 2242 - was about to head to sleep and try again, when i had the idea to check whether $ accounts could login mssql  
  
2243 - logged in with the box's $ account on an mssql instance on another box - not only did it work, it had impersonation privs!  
2244 - and if i took up the impersonation, i had a linked login!  
2245 - confirmed linked server!  
2253 - checked priv on linked server - '沙'!!!!!  
2254 - without checking if the linked server had a link-back, or even if the link-back would execute with '沙', attempted mssql link-back code exec first step of enabling "show advanced options" - it worked!!!!!!  
2255 - enabled xp_cmdshell via link-back  
2256 - 2259 - got shell as mssql user on new box!  
2300 - compiled runspace pipepipe  
2301 - setup first stage of 别林斯布负了 by listening with pipepipe  
2302 - triggered 斯波三倍了 to complete second stage of 别林斯布负了, pipepipe executed powershell runner  
...
2310 - grabbed 洛克了 (no 布鲁夫 altho admin)  

# 旗 9
2316 - used 电脑厨师 to powershell-base64-encode a powershell cradle  
2318 - opened ssms and immediately executed mssql link code exec on the next box  
2318 - met called  
2320 - tried 别林斯布负了 to privesc - listening with pipepipe  
2321 - 2327 - but 别林斯布负了 failed. i wouldn't know why not, would discover later it's cos spooler service was disabled on the box.  
...
0542 - wondered if 终点 box could use nearby db to tunnel out to me, by setting up chisel link between nearby db and db closest to me  
...
0746 - checked out some apparently important wp 秘密 file, but was gibberish  
...
0824 - 1244 - found no routes into 终点 box, and had convinced myself that webshell was a dead end. heavily considered resigning to fate.    
...
1554 - obviously mini蛋 wouldn't work without admin priv - only obvious choice was to choose to use pipepipe to run mini蛋 with system priv, if that would even work - but i had to give it a try  
1559 - ..and then it actually worked  
1559 - it actually worked!!!  
1559 - took another screenshot of 斯波三倍了 output just for security  
1559 - 勒索啥.对面破 with 47mb now just sitting on the box - but i couldn't download since it was created by system, and i was just a normal service user  
1610 - tried using pipepipe to run icacls to change 勒索啥.对面破 permissions  
1614 - pipepipe ran as intended - but still not enough privs  
1616 - then another brainwave - why not create 洛克了 admin user, a la normal service abuse? wrote pipepipe to do this  
1618 - pipepipe ran as intended  
1620 - just now was creating jack, now was adding jack to 洛克了 admin group  
...
1711 - used pipepipe to killdef  
1716 - realized even when logged in as jack with 洛克了 admin membership, i was still low-priv. very effective uac at work - so again consulted stackoverflow and tried icacls to grant privs on 勒索啥.对面破  
1735 - was when i 终点ly had the biggest brainwave - write a powershell script as 洛克了 admin, then simply make pipepipe keep running that script with system privs  
1737 - worked  
1755 - made myself RDP into the 终点 box - surreal  
1755 - captured screenshot of my script just for security   
1757 - saw 秘密, but didn't open it since i was on RDP, and that wouldn't count  
1804 - impacket-psexec in with admin hash, grabbed 秘密
