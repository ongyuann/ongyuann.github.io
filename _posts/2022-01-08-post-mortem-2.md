...
1208 - still chose not to act on the vector - tried looking at blood狗 results to see if i could move via domain rights. couldnt.  
...
1453 - ran runspace hollow via impacket-psexec 壳 on the logical target  
...
1510 - logged in as other user via 弱点破, used the ssh keys successfully  
...
1727 - add current user as 洛克了 admin and 弱点破 user on the box  
...
2225 - tried looking deeper into blood狗 for any avenues - no luck  
2227 - tried looking deeper via 权力view - here i was enumerating domain groups for any possible privs blood狗 might have missed  
...
2308 - setup 弱点破, used first box as jumphost, used mimikatz to pass admin hash and open 弱点破 restrictedadmin  
...
0542 - wondered if 终点 box could use nearby db to tunnel out to me, by setting up chisel 连 between nearby db and db closest to me  
0546 - setup chisel 连s between said dbs - would give up this avenue after thinking twice  
0554 - tried 字按scan - failed

0657 - was where i realized i could replace the 字按 admin user's password hash with my own, so generated a 字按 hash of "P@ssw0rd"  
0659 - updated 字按 database with my admin password  
...
0712 - inserted php web壳 code and got code exec  
...
0821 - could upload file.txt and certutil -decode successfully via web壳  
0823 - saw that i could reliably upload files via web壳  

0824 - 1244 - found no routes into 终点 box, and had convinced myself that web壳 was a dead end. heavily considered resigning to fate.    
...
1554 - obviously mini蛋 wouldn't work without admin priv - only obvious choice was to choose to use 管道管道 to run mini蛋 with 系统 priv, if that would even work - but i had to give it a try  
...
1616 - then another brainwave - why not create 洛克了 admin user, a la normal service abuse? wrote 管道管道 to do this  
...
1643 - 管道管道 ran as intended and added jack to remote management users  
...
1735 - was when i 终点ly had the biggest brainwave - write a 权力壳 script as 洛克了 admin, then simply make 管道管道 keep running that script with 系统 privs  
1737 - worked  
