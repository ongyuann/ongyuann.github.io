...
```
Now we have some filenames inside `Carl`'s folder in the `Secure$` share, and on his `Desktop` folder. We can also reasonably conclude that `Carl` is `c.smith`.
```
...
### SMB as Carl
The main challenge here was that the `Utils.vb` script is written in VBScript (i.e. it was a challenge to me). To make lives easier for ourselves, we can convert everything to `C#` using the [super-useful Telerik code converter](https://converter.telerik.com/). That's right - just go to the link and dump the contents of `Utils.vb` in there, and you'll have the script re-written in `C#`.
